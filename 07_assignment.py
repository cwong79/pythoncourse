...
def exercise02(new_observations):
    ...
    iris_num_predictions = knn.predict(new_observations)
    iris_predictions = iris.target_names[iris_num_predictions]
...
def exercise04():
    ...
    X_train, X_test, y_train, y_test = tts(X, y, test_size=.25, random_state = 21)
    k_range = range(1, 30)
    scores = []
    ...
    for k in k_range:
        ...
        performance_score = scores.append(knn.score(y_test, y_pred))
    
    plt.plot(k_range, performance_score)
    plt.xlabel('Value of K for KNN')
    plt.ylabel('Testing Accuracy')
    
    plotly_overfit_underfit_curve_url = plt.show()
...
def exercise05():
    ...
    medv_as_numpy_values = Boston["target"].apply(np.mean)
...
def exercise06():
    ...
    Boston = pd.DataFrame(np.c_[boston['data'], boston['target']], columns= np.append(boston['feature_names'], ['target']))
    plt.scatter(Boston['PTRATIO'], Boston['target'])
    plt.xlabel('PTRATIO')
    plt.ylabel('MEDV')
...
def exercise07():
    ...
    reg_model = reg.fit(x.reshape(-1, 1), y)
