...
'''
    ...
    class Box:
        ...
        def combine(self, other):
            return Box(*(sum(pair) for pair in zip(self.width, other.width)))
...
def exercise02():
    ...
    class MangoDB(object):
        def __init__(self):
            collection = {'default':  {'version':1.0, 'db':'mangodb', 'uuid':str(uuid.uuid4())}}
            self.collection = collection
        
        def display_all_collections(self):
            for key_prim in self.collection:
                print(key_prim)
                for key_sec in self.collection[key_prim].keys():
                    print(key_sec, ":", self.collection[key_prim][key_sec])
        
        def add_collection(self, collection_name):
            self.collection[collection_name] = {}
        
        def update_collection(self, collection_name, updates):
            for key_sec in updates.keys():
                self.collection[collection_name][key_sec] = updates[key_sec]            
        ...
        def to_json(self, collection_name):
            return(json.dumps(self.collection[collection_name]))
    ...
    for i in range(len(test_scores)):
        dico[i+1] = test_scores[i]
    db.update_collection('test_scores', dico)
    print(db.get_collection_size('test_scores'))
    db.display_all_collections()
    print(db.collection['default']['uuid'])
    db.wipe()
    print(db.collection['default']['uuid'])
