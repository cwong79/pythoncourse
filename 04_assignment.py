...
class PandasChain:
    ...
    def display_chain(self): 
        for block in self.__chain:
            print(f"The information for block id {block.__seq_id} is:\n")
            print(f"Status : {block.__status}\n")
            print(f"Block Hash : {block.__block_hash}\n")
            print(f"Previous Block Hash : {block.__prev_hash}\n")
            print(f"Merkle Hash : {block.merkle_tx_hash}\n")
            # print(f"All of transactions in this block : {'->'.join(block.__transactions["Value"].values)}\n\n")
    ...
    def __commit_block(self,block): 
        # Add code here
        new_block = Block(block.__seq_id, block.__block_hash)
        block.set_status("COMMITTED")
        new_block.__merkle_root = new_block.get_simple_merkle_root()
        block_hash = hashlib.sha256(str(str(uuid.uuid4()))) + str(new_block.__seq_id) # Create block hash
    ...
    def display_block_headers(self): 
         for block in self.__chain:
             print(f"Block id {block.__seq_id} is:\n")
             print(f"Status : {block.__status}\n")
             print(f"Block Hash : {block.__block_hash}\n")
             print(f"Previous Block Hash : {block.__prev_hash}\n")
             print(f"Merkle Hash : {block.merkle_tx_hash}\n")
             print(f"Total Number of transactions : {block.__transactions.shape[0]}\n\n")
    ...
    def get_values(self):
        values = []
        for block in self.__chain:
            values.extend(block.__transactions["Value"].values)
        return values
...
class Block:
    ...
    def add_transaction(self,s,r,v): 
        ...
        tx_hash = hashlib.sha256(str(ts).encode('utf-8') + s.encode('utf-8') + r.encode('utf-8') + str(v).encode('utf-8')).hexdigest() # Hash of timestamp, sender, receiver, value
    ...
    def set_status(self,status):
        if self.__transactions.shape[0] == 10:
    ...
    def get_simple_merkle_root(self): 
        return ''.join(self.__transactions["TxHash"].values)
    
    def get_values(self):
        return self.__transactions["Value"].values

    def get_timestamps(self):
        timestamps = []
        for block in self.__chain:
                timestamps.extend(block.__transactions["Timestamp"].values)
        return timestamps
