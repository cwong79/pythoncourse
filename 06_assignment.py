...
def exercise03(a):
    ...
    # ------ Place code below here \/ \/ \/ ------
    def sumOfndarray(ndarray):
    
        result = 0
    
        for rows in range (len(ndarray)):
        
            for cols in range(len(ndarray[0])):
            
                result = result + ndarray[rows][cols]
...
def exercise08():
    ...
    avg_sq_ft = df['sq__ft'].mean() #Average square feet across all transactions
...
def exercise09():
    ...
    r = requests.get('https://api.coindesk.com/v1/bpi/historical/close.json?start=2017-09-01&end=2018-10-05')
    with open('r', 'r') as f:
        data = json.load(f)
...
def exercise12(n):
    ...
    checkerboard_matrix[0::1, ::1] = 1
    checkerboard_matrix[::1, 0::1] = 1
...
def exercise13(n):
    ...
    def random_dates(start, n, freq, seed=None):
        if seed is not None:
            np.random.seed(seed)
    ...
    dr = pd.date_range(start, freq=freq)
    return pd.to_datetime(np.sort(np.random.choice(dr, n, replace=False)))
    
    random_dates('2015-01-01', 10, 'H', seed=[3, 1415])
