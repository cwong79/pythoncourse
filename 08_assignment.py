...
class Iris:
    def __init__(self, creds, dbname='data602', new=True):
        self.__conn = mc.connect(user=creds['user'], password=creds['password'], host='127.0.0.1')
    ...
    def load(self,truncate=False):
        ...
        target_species_values = ['setosa', 'versicolour', 'virginica']
        iris = ds.load_iris()
        x_data = iris.data
        sql_insert = '''INSERT INTO {0}.iris_data (id,feature_sepal_length,feature_sepal_width,feature_petal_length,feature_petal_width,target_species,target_species_id) 
                        VALUES (%s,%s,%s,%s,%s,%s,%s); commit;
                     '''.format(self.__dbname)
        sql_data = []
        for idx, item in enumerate(x_data):
            sepal_length = float(item[0])
            sepal_width = float(item[1])
            petal_length = float(item[2])
            petal_width = float(item[3])
            target_species_id = int(iris.target[idx])
            target_species = target_species_values[target_species_id]

            sql_data.append(
                (idx, sepal_length, sepal_width, petal_length, petal_width, target_species, target_species_id))

        if not self.__conn.is_connected():
            self.__conn.connect()

        mycursor = self.__conn.cursor()
        mycursor.executemany(sql_insert, sql_data)
        mycursor.close()
    ...
    def display_gt(self,n): 
        ...
        sql_str = '''SELECT * FROM {0}.iris_data WHERE id>{1}'''.format(self.__dbname, n)
    ...
    def update_observation(self,id,new_target_species,new_target_species_id):
        ...
        sql_update = ''' UPDATE {0}.iris_data 
                         SET target_species = '{1}', target_species_id = {2}
                         WHERE id ={3}; commit;
                     '''.format(self.__dbname,new_target_species,  new_target_species_id, id)
    ...
    def del_observations(self,row_ids):
        ...
        sql_delete = ''' DELETE FROM {0}.iris_data
                         WHERE id in {1}
                      '''.format(self.__dbname,tuple(row_ids))
        if not self.__conn.is_connected():
            self.__conn.connect()
    ...
    def __truncate_iris(self):
        ...
        sql_str = '''TRUNCATE TABLE {0}.iris_data'''.format(self.__dbname)
        if not self.__conn.is_connected():
            self.__conn.connect()
    ...
    def get_row_count(self):
        ...
        sql_str = '''SELECT COUNT(*) FROM {0}.iris_data '''.format(self.__dbname)
        if not self.__conn.is_connected():
            self.__conn.connect()
        cursor = self.__conn.cursor()
        cursor.execute(sql_str)
        ...
        cursor.close()
        print('Row count is {0}'.format(result[0]))
        count = result[0]
